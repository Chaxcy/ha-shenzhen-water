...
class ShenzhenWaterConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_sms(self, user_input=None):
        """Step 2: input SMS code, login, fetch customer code, ctoken and latest bill month."""
        errors: dict[str, str] = {}
            ...
            if not self._mobile:
                ...
                try:
                    ...
                    if not openid or not guid or not utoken:
                        _LOGGER.error("Invalid LoginV20 data: %s", login_data)
                    ...
                    _LOGGER.exception(
                        "Network error during Shenzhen Water login: %s",
                        err,
                    )
                    errors["base"] = "cannot_connect"
                    ...
                    _LOGGER.exception(
                        "Unexpected error during Shenzhen Water login: %s",
                        err,
                    )
                    errors["base"] = "unknown"
